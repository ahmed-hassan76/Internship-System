// Student Registration & Login with Redirect Handling
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginForm) {
      loginForm.addEventListener("submit", (e) => {
        ...
        const users = JSON.parse(localStorage.getItem("users")) || [];
        const match = users.find(u => u.email === email && u.password === password && u.role === "student");
  
        const params = new URLSearchParams(window.location.search);
        const nextPage = params.get("next") || "student-dashboard.html";
